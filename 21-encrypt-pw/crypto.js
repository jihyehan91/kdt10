...
function saltAndHashPW(password) {
  ...
  // hash 생성 => slat랑 password를 결합해서 해시를 생성
  const hash = crypto
    .pbkdf2Sync(password, salt, iteration, keylen, algorithm)
    .toString('base64'); // buffer -> string 반환
...
// 비밀번호 확인
const inputPW = '1234';
const isMatch = checkPW(inputPW, salt, hash); // true or falst 반환
console.log(`비밀번호 일치 여부 확인: ${isMatch}`);
